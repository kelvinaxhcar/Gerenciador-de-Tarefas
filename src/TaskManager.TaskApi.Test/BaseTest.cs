    ...
    services.AddFluentMigratorCore()
        .ConfigureRunner(
            rb => rb.AddSQLite()
                      ...
                      .ScanIn(typeof(_2024081502_create_task_table).Assembly)
                      .For.Migrations())
  ...
  public void DeleteDatabase(string dbName) {
    try {
      if (File.Exists(dbName)) {
        File.Delete(dbName);
        Console.WriteLine(
            $"Banco de dados '{dbName}' foi deletado com sucesso.");
      } else {
        Console.WriteLine(
            $"O arquivo de banco de dados '{dbName}' não existe.");
      }
    } catch (IOException ex) {
      Console.WriteLine(
          $"Erro ao tentar deletar o banco de dados: {ex.Message}");
    } catch (UnauthorizedAccessException ex) {
      Console.WriteLine(
          $"Acesso negado ao tentar deletar o banco de dados: {ex.Message}");
    }
  }
}
}
