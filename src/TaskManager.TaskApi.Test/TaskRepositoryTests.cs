...
public class TaskRepositoryTests : BaseTest
{
    ...
    {
        var serviceProvider = CreateServices(nameof(Add_Task_Should_Add_Task_In_Database));
    ...
    {
        var serviceProvider = CreateServices(nameof(Delete_Task_Should_Delete_Task_In_Database));

        using var scope = serviceProvider.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<AppDbContext>();
        var repository = scope.ServiceProvider.GetRequiredService<TaskRepository>();

        var task = new TaskApi.Domain.Models.Task()
        {
    ...
    {
        var serviceProvider = CreateServices(nameof(Get_All_Tasks_Must_Get_All_Tasks_From_Database));

        using var scope = serviceProvider.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<AppDbContext>();
        var repository = scope.ServiceProvider.GetRequiredService<TaskRepository>();

        var task1 = new TaskApi.Domain.Models.Task()
        {
            Description = "Test desc1",
            Title = "Test1"
        };

        var task2 = new TaskApi.Domain.Models.Task()
        {
    ...
    {
        var serviceProvider = CreateServices(nameof(Get_By_Id_Should_Get_Task_By_Id_From_Database));
    ...
    {
        var serviceProvider = CreateServices(nameof(Edit_Task_Must_Edit_Task_Database));
        ...
        await repository.EditAsync(task2.Id, new TaskDTO()
        {
