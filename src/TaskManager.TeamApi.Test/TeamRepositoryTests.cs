...
public class TeamRepositoryTests : BaseTest {
  ...
  public async void AddTeam_Should_Add_Team_In_Database() {
    var serviceProvider =
        CreateServices(nameof(AddTeam_Should_Add_Team_In_Database));
  ...
  public async void Delete_Team_Should_Delete_Team_In_Database() {
    var serviceProvider =
        CreateServices(nameof(Delete_Team_Should_Delete_Team_In_Database));

    using var scope = serviceProvider.CreateScope();
    var context = scope.ServiceProvider.GetRequiredService<AppDbContext>();
    var repository = scope.ServiceProvider.GetRequiredService<TeamRepository>();

    var team = new Team() { Description = "Test desc", Name = "Test" };
  ...
  public async void Get_All_Teams_Must_Get_All_Teams_From_Database() {
    var serviceProvider =
        CreateServices(nameof(Get_All_Teams_Must_Get_All_Teams_From_Database));

    using var scope = serviceProvider.CreateScope();
    var context = scope.ServiceProvider.GetRequiredService<AppDbContext>();
    var repository = scope.ServiceProvider.GetRequiredService<TeamRepository>();

    var team1 = new Team() { Description = "Test desc1", Name = "Test1" };

    var team2 = new Team() { Description = "Test desc2", Name = "Test2" };
  ...
  public async void Get_By_Id_Should_Get_Team_By_Id_From_Database() {
    var serviceProvider =
        CreateServices(nameof(Get_By_Id_Should_Get_Team_By_Id_From_Database));
  ...
  public async void Edit_Team_Must_Edit_Team_Database() {
    var serviceProvider =
        CreateServices(nameof(Edit_Team_Must_Edit_Team_Database));
    ...
    await repository.EditAsync(
        team2.Id, new TeamDTO() { Description = "Test desc3", Name = "Test3" });

    var team =
        await context.Team.Where(x => x.Id == team2.Id).FirstOrDefaultAsync();
